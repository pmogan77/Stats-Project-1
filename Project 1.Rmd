...
```{r}
# Determine which combination of licensors and producers have the most members (uses pivot_longer and pivot_wider)
producerLicensorData = mergedData %>% group_by(Producers, Licensors) %>% summarise(TotalMembers = sum(Members)) %>% arrange(-TotalMembers) %>% filter(!is.na(Producers) && !is.na(Licensors) && Licensors != "Unknown" && Producers != "Unknown")
producerLicensorData
producerLicensorDataUntidy = producerLicensorData %>% pivot_wider(values_from = TotalMembers, names_from = Licensors)
producerLicensorDataUntidy
producerLicensorDataUntidy %>% pivot_longer(producerLicensorData$Licensors, names_to = "Licensors", values_to = "TotalMembers") %>% filter(!is.na(TotalMembers))
```
